...
<html lang="en">
    <head>
        ...
        <title>Payment Approval Management System || Department Request</title>
                            ...
                            <div class="col-lg-12 stretch-card"> -->
                                <div class="card">
                                    ...
                                    <div class="table-responsive pt-3">
                                        <table class="table table-bordered table-hover" id="req-dept">
                                            <thead>
                                                <tr class="table-info">
                                                    <th>No.</th>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            @elseif($request->status=="Rejected")
                                                                @if($request->nextApp == NULL)
                                                                    Rejected by Finance
                                                                @else
                                                                    Rejected by {{$request->applicationTracking->sortByDesc('created_at')->first()->approver->role->RoleName}}
                                                                @endif
                                                            @elseif($request->status=="Approved")
                                                                Approved by {{$request->applicationTracking->sortByDesc('created_at')->first()->approver->role->RoleName}}
                                                            ...
                                                            @elseif($request->status=="Requiring Attention")
                                                                ...
                                                                @else
                                                                    Requiring Attention by {{$request->applicationTracking->sortByDesc('created_at')->first()->approver->role->RoleName}}
                                                                @endif
                                                            @else
                                                                Error 404
                                                            @endif
                                                        </td>
                                                        <td class="text-center"><a href="/applications/{{$request->id}}" class="btn btn-info btn-sm rounded-pill"><b>View</b></a></td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
